...
class Login extends React.Component {
    ...
    //mkkkount nowss
    componentDidMount(){
      var isUserLoggedIn = sessionStorage.getItem("TROLLTOWER_USER")
      var tokenAvail = sessionStorage.getItem("TROLLTOWER_TOKEN")
    ...
    async submitLogin(){
      try {
        ...
        if(response.data.status){
          await this.setState({notification:{status: "success", message: response.data.message}, loading: false});
          auth.setSessionStorage(response)
          setTimeout(()=>{
            return this.props.history.push("/admin/dashboard")
          }, 500)
        }else{
          return this.setState({notification:{status: "danger", message: response.data.message}, loading: false});
        }
